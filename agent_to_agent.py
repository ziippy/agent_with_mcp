...
class MCPServerConnection:
    ...
    name: str
    toolset: McpToolset
    description: str = ""  # .env에서 읽은 서버 설명
...
class QuestionUnderstandingAgent(SpecializedAgent):
    """Agent A: 질문 이해 및 라우팅 담당 (A2A 오케스트레이터)"""

    def __init__(self, llm_client: LLMClient, available_agents: List[str], agent_descriptions: Dict[str, str]):
        """
        Args:
            llm_client: LLM 클라이언트
            available_agents: 사용 가능한 에이전트 목록 (예: ["laws", "precedent", "search"])
            agent_descriptions: 각 에이전트의 역할 설명 {"laws": "법률 검색 전문 에이전트", ...}
        """
...
- 예시:
  * "최근 판례를 찾고, 그 판례의 법률 조항을 알려줘" → [["search"], ["laws"]] ✅
...
{{
  ...
  "dependencies": {{
    "agent2": "agent1의 결과를 어떻게 활용할지" (순차 실행 시에만)
  }},
  "analysis": "병렬/순차 판단 이유"
}}

**중요: 질문에 여러 주제/관점이 있으면 기본적으로 병렬 실행하세요!**

**예시 1 - 병렬 (가장 일반적):**
질문: "12대 중과실이 뭐야?"
→ 법률 정의도 필요하고, 실제 사례/판례도 도움이 됨
```json
{{
...
**예시 2 - 순차 (명확한 의존성):**
질문: "최근 부당해고 판례를 찾고, 그 판례에서 인용된 법 조항을 알려줘"
→ 반드시 판례를 먼저 찾아야 법 조항을 알 수 있음
```json
{{
  "question_type": "sequential",
  "execution_order": [["search"], ["laws"]],
  "queries": {{
    "search": "최근 부당해고 관련 판례를 찾아주세요",
    "laws": "판례에서 인용된 법률 조항을 찾아주세요"
  }},
  "dependencies": {{
    "laws": "search가 찾은 판례에서 인용된 법률 조항을 검색"
  }},
  "analysis": "판례를 먼저 찾은 후, 그 결과를 바탕으로 법률 조항을 검색해야 하므로 순차 실행"
}}
```
...
class ToolBasedAgent(SpecializedAgent):
    """도구 기반 전문 에이전트 (범용) - A2A 지원"""

    def __init__(self, name: str, role: str, description: str, llm_client: LLMClient, tools: List[BaseTool],
                 orchestrator: Optional['MultiAgentOrchestrator'] = None):
        system_prompt = f"""당신은 {role}입니다.
...
- 사용자의 요청을 최선을 다해 완수"""
        ...
        self.orchestrator = orchestrator  # A2A를 위한 오케스트레이터 참조
    ...
    async def process_with_tools(self, user_input: str, context: Optional[Dict[str, Any]] = None) -> AgentResponse:
        ...
        # .env에서 MAX_ITERATIONS 읽기 (기본값: 10)
...
class MultiAgentOrchestrator:
    ...
    async def connect_mcp_server(self, server_name: str, base_url: str, auth_bearer: str = "", description: str = "") -> MCPServerConnection:
        ...
        try:
            ...
            connection = MCPServerConnection(
                name=server_name,
                toolset=toolset,
                description=description  # .env에서 읽은 설명 저장
            )
    ...
    def initialize_agents(self):
        ...
        for server_name in self.servers.keys():
            ...
            # .env의 DESCRIPTION이 있으면 우선 사용, 없으면 자동 생성
            if server.description:
                description = server.description
                print(f"[INFO] Using .env description for '{server_name}': {description}")
            else:
                # 도구들의 description을 기반으로 에이전트 역할 설명 자동 생성
                description = self._generate_agent_description(server_name, server_tools)
                print(f"[INFO] Auto-generated description for '{server_name}': {description}")

            agent_descriptions[server_name] = description

        # Agent A 초기화 (라우팅 에이전트) - 에이전트 설명만 전달 (A2A 방식)
        available_agents = list(self.servers.keys())
        self.question_agent = QuestionUnderstandingAgent(self.llm_client, available_agents, agent_descriptions)
        ...
        for server_name in self.servers.keys():
            ...
            # 에이전트 생성 (A2A 방식 - 설명 포함)
            description = agent_descriptions[server_name]
            agent = ToolBasedAgent(
                name=f"{server_name.upper()}Agent",
                role=f"{server_name} 전문 에이전트",
                description=description,
                llm_client=self.llm_client,
                tools=server_tools,
                orchestrator=self  # A2A를 위한 오케스트레이터 참조
            )
            self.specialist_agents[server_name] = agent
            print(f"   • {agent.name}: {description}")
            print(f"      도구: {len(server_tools)}개")

    def _generate_agent_description(self, server_name: str, server_tools: List[BaseTool]) -> str:
        """도구들의 description을 기반으로 에이전트 역할 설명 자동 생성 (범용)"""
        if not server_tools:
        ...
        for tool in server_tools:
            tool_name = getattr(tool, 'name', '').replace(f'{server_name}__', '')
        ...
        if len(tool_descriptions) <= 3:
            ...
            return f"{server_name} 전문 에이전트. 제공 기능:\n{tools_summary}"

        # 도구가 많으면 LLM을 사용하여 요약
        try:
            tools_list = "\n".join(tool_descriptions[:10])  # 최대 10개까지만
            if len(tool_descriptions) > 10:
                tools_list += f"\n... 외 {len(tool_descriptions) - 10}개 도구"

            # 간단한 프롬프트로 요약 생성
            prompt = f"""다음은 '{server_name}' MCP 서버가 제공하는 도구 목록입니다:

{tools_list}

위 도구들의 기능을 종합하여, 이 에이전트가 무엇을 전문으로 하는지 1-2문장으로 간결하게 설명해주세요.
형식: "[도메인/기능] 전문 에이전트. [주요 역할 설명]"

예시: "법률 검색 전문 에이전트. 법령, 판례, 조문을 검색하고 해석합니다."
"""
            ...
            # 생성된 설명이 너무 길면 줄임 (200자 제한)
            if len(generated_desc) > 200:
                generated_desc = generated_desc[:197] + "..."

            return generated_desc

        except Exception as e:
            # LLM 호출 실패 시 폴백: 도구 이름 나열
            print(f"[WARN] Failed to generate description with LLM: {e}")
            tool_names = [getattr(tool, 'name', '').replace(f'{server_name}__', '') for tool in server_tools[:5]]
            tool_summary = ', '.join(tool_names)
            if len(server_tools) > 5:
                tool_summary += f" 외 {len(server_tools) - 5}개"
            return f"{server_name} 전문 에이전트 ({tool_summary} 제공)"
...
async def initialize_multi_agent() -> MultiAgentOrchestrator:
    ...
    try:
        ...
        while True:
            ...
            description_key = f"MCP_SERVER_{server_index}_DESCRIPTION"
            ...
            # ENABLED 체크 (기본값: true)
            server_enabled = os.environ.get(enabled_key, "true").lower()
            ...
            server_name = os.environ.get(name_key, f"mcp{server_index}")
            server_description = os.environ.get(description_key, "")  # .env에서 설명 읽기

            print(f"Connecting to MCP Server '{server_name}': {server_url}")
            try:
                await orchestrator.connect_mcp_server(server_name, server_url, server_bearer, server_description)
